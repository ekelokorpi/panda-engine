...
//the own renderer
    ...
    function PixiRender(proton, element) {
    ...
    PixiRender.prototype.onParticleCreated = function(particle) {
        ...
        particleSprite.blendMode = this.proton.blendMode || 0;
        particle.sprite = particleSprite;
        var target = this.proton.container || game.system.stage;
        target.addChild(particle.sprite);
    }
    ...
    PixiRender.prototype.onParticleDead = function(particle) {
        var target = this.proton.container || game.system.stage;
        target.removeChild(particle.sprite);
    }
...
game.ProtonEngine = game.Class.extend({
    init: function() {
        this.engine = new Proton();
        this.renderer = new Proton.Renderer('pixi', this.engine);

        this.emitter = new Proton.Emitter();
        // this.emitter.rate = new Proton.Rate(this.rate.num, this.rate.time);
        // this.emitter.addInitialize(new Proton.Life(this.life.min, this.life.max));
        // this.emitter.addInitialize(new Proton.Velocity(this.vel.radius, this.vel.angle, this.vel.type));
        // this.emitter.p.x = this.pos.x;
        // this.emitter.p.y = this.pos.y;
    },

    setTextures: function(textures) {
        var arr = [];
        for (var i = 0; i < textures.length; i++) {
            arr.push(new PIXI.Texture.fromImage(textures[i]));
        };
        if(arr.length == 0) return false;
        this.emitter.addInitialize(new Proton.ImageTarget(arr));
    },

    start: function(emitTime, life) {
        this.emitter.emit(emitTime, life);
        this.engine.addEmitter(this.emitter);
        this.renderer.start();
    },
